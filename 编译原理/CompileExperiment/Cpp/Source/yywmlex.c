...
#else
YYEXPFUNC(yywint_t) YYCDECL yywmlex(yy)
yywmlexer_t YYFAR *yy;
#endif
{
	yyassert(yy != NULL);
	yyassert(yywmisfastlexer(yy));	/* make sure it's a fast lexical analyser */
	...
	while (1) {
		...
		do {
			...
			while (state != 0) {
				long index = yy->yymstate[state].yymbase + ch;
				if (index >= 0 && index < yy->yymtransitionmax) {
					if (yy->yymtransition[index].yymcheck == state) {
						state = yy->yymtransition[index].yymnext;
