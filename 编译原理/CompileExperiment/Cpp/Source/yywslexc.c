...
#endif
{
	while (1) {
		...
		do {
			...
			index = yywstate[state].yybase;
			while (1) {
				if (yywctransition[index].yynext == -1) {
				...
				if (ch >= yywctransition[index].yyfirst &&
					ch <= yywctransition[index].yylast) {
					state = yywctransition[index].yynext;
					break;
				}
				index++;
			}
