...
void print(int pointer) {
    for (int i = pointer; i < parsedString.size(); ++i)
        putchar(parsedString[i]);
}

void stackPrint() {
    stack<char> newStack;
    while (!parsedStack.empty()) {
        newStack.push(parsedStack.top());
        parsedStack.pop();
    }
    while (!newStack.empty()) {
        putchar(newStack.top());
        parsedStack.push(newStack.top());
        newStack.pop();
    }
}

void analyseStackPrint(Grammer grammer, int pointer) {
    cout << counter++ << "\t\t";
    stackPrint();
    cout << "\t\t";
    print(pointer);
